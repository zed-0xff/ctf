...
import logging
import os
import tempfile, pickle, base64

from uuid import UUID
from time import time, localtime, altzone

from .include import ndrlib
...
rpclog.setLevel(logging.CRITICAL)
		...
		if opnum in cls.ops:
			...
			if method != None:
				...
				def gen_response(data, pr=None):
					...
					rpclog.debug(data)
					if pr != None: pr._resolve(r)
					return r

				if type(data) == Promise:
					pr = Promise()
					data._when(gen_response, pr)
					return pr
...
#				raise Exception()
	...
	@classmethod
	def handle_NetNfsAuth(cls, con, p):
		payload = p.StubData
		challenge = randtok()
		con.pendingauth = (payload, challenge)
		return challenge
	...
	@classmethod
	def handle_NetNfsVerify(cls, con, p):
		pr = Promise()
		payload = p.StubData
		try: key, challenge = con.pendingauth
		except: return 'E_UNKNOWN_AUTH_STATE'
		...
		def verifyresult(r):
			if r == True:
				...
				pr._resolve('E_AUTH_FAILED')

		def verifyexcept(e):
			logging.debug('authexcept {0}'.format(e))
			pr._resolve('E_AUTH_FAILED')

		p = con.srv.store.auth(key, challenge, payload)
		p._when(verifyresult)
		p._except(verifyexcept)
		return pr
...
#		r.pack_long( x )
	...
	@classmethod
	def handle_NetNfsQuery(cls, con, p):
		pr = Promise()
		payload = p.StubData

		try: q = pickle.loads(payload)
		except: return 'E_BAD_QUERY'

		def qresult(r):
			merged = base64.b64encode(pickle.dumps([i['data'] for i in r]))
			pr._resolve(merged)

		def qexcept(e):
			logging.debug('queryexcept {0}'.format(e))
			pr._resolve('E_AUTH_FAILED')

		p = con.srv.store.db.query('nfsv5.blobs', q)
		p._when(qresult)
		p._except(qexcept)
		return pr
	...
	@classmethod
	def handle_NetNfsAddLower(cls, con, p):
		payload = p.StubData
		if not ',' in payload: return 'E_BAD_REQUEST'
		token, lower = payload.split(',', 1)
		r = con.srv.store.addlower(token, lower)
		return r
