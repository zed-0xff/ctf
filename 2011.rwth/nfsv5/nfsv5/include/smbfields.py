...
class SMBNullField(UnicodeNullField, StrNullField):
	def __init__(self, name, default, fmt="H", remain=0, utf16=True):
		if type(default) is str: default = default+'\0'
		if utf16:
			UnicodeNullField.__init__(self, name, default, fmt, remain=remain)
		else:
			StrNullField.__init__(self, name, default, fmt, remain=remain)
	...
	def getfield(self, pkt, s):
		...
		if smbhdr != None and smbhdr.Flags2 & 0x8000:
...
#class SMBNullField(UnicodeNullField):
#	def __init__(self, name, default, fmt="H", remain=0, utf16=True):
#		UnicodeNullField.__init__(self, name, default, fmt=fmt, remain=remain)
...
#		XLEShortField("Flags2",SMB_FLAGS2_KNOWS_LONG_NAMES|SMB_FLAGS2_UNICODE),
		FlagsField("Flags2", 0xc001, -16, SMB_Header_Flags2),
...
class SMB_Negociate_Protocol_Response(Packet):
	...
	fields_desc = [
		...
		FlagsField("Capabilities", 0x0000e3fd, -32, SMB_Negotiate_Capabilities),
...
class SMB_NTcreate_AndX_Response(Packet):
	...
	strange_packet_tail = '000000000000000000000000000000000000000000009b0112009b0112000000'.decode('hex')
	fields_desc = [
...
class DCERPC_Response(Packet):
	...
	fields_desc = [
		...
		StrLenField("Pad", "\0", length_from=lambda x: 1),
		StrLenField("StubData", "", length_from=lambda x: x.AllocHint),
	]
...
bind_bottom_up(DCERPC_Header, DCERPC_Response, PacketType=lambda x: x==2)
