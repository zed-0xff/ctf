...
static int store_flag(const char * token, const char * flag)
{
	char path[256];
	int fd;
...
static int retrieve_flag(const char * token, char * flag)
{
	int res;
	char path[256];
	int fd;
...
static int send_response(char * response, size_t length)
{
	register int res;
...
int main()
{
	static const struct sockaddr_in srv_addr = {
		.sin_family = AF_INET,
		.sin_addr = { INADDR_ANY },
	...
	if((g_page_size = getpagesize()) < sizeof(buf))
		return -1;
	
	if(signal(SIGCHLD, SIG_IGN) < 0)
	{
		ERROR("Failed to discard SIGCHLD");
		return -2;
	}

	if((g_sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0)
	{
		ERROR("Failed to create socket");
		return -1;
	}

	if(bind(g_sock, (struct sockaddr *) &srv_addr, sizeof(srv_addr)) < 0)
	{
		ERROR("Failed to bind to udp/9000");
		return -1;
	}

	for(;;)
	{
		int addr_size = sizeof(g_remote_addr);
		pid_t pid;

		if((res = recvfrom(g_sock, buf, sizeof(buf), 0, (struct sockaddr *) &g_remote_addr, &addr_size)) < 0)
		{
		...
		MESSAGE("Received %u bytes payload from %s:%hu", res,
			inet_ntoa(g_remote_addr.sin_addr), ntohs(g_remote_addr.sin_port));

		pid = fork();

		if(pid < 0)
		{
			perror("Failed to fork for child");
